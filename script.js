...
searchModal.onclick = function () {
  modal.style.display = "block";
  searchInput.value = searchInpuValue;
};
...
function resetModal() {
  ...
  searchInputValue = ""; // Reset the stored search input value
  currentEditId = null;
  const searchButton = document.getElementById("searchButton");
  const searchInput = document.getElementById("searchInput");

  searchButton.onclick = function () {
    searchInputValue = searchInput.value; // Store the search input value
    searchForFood(searchInputValue);
  };
}
...
function displayFoods(foods) {
  modalContent.innerHTML =
    '<span class="close">&times;</span><h2>Search Results:</h2>';
  ...
  try {
    ...
    if (currentEditId) {
      response = await axios.put(
        `https://ego-quest.herokuapp.com/users/647819fceee9cc64b271b635/food-entries/${currentEditId}`,
        {
          foodName,
          foodItemId,
          user: userId , 
    ...
    response = await axios.post(
      "https://ego-quest.herokuapp.com/users/647819fceee9cc64b271b635/food-entries",
      {
        foodName,
        foodItemId,
        user: userId,
...
async function displayFoodEntries() {
  try {
    ...
    entries.forEach((entry) => {
      ...
      row.innerHTML = `
        <td><button onclick="editEntry('${entry._id}')">Edit</button></td>
...
async function deleteEntry(id) {
  try {
    await axios.delete(
      `https://ego-quest.herokuapp.com/users/647819fceee9cc64b271b635/food-entries/${id}`
    );
